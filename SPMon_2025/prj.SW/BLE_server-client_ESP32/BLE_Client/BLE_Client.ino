...
static bool BLE_ClientMgr_connectToServer(BLEAddress pAddress);
/* When the BLE Server sends a new reading with the notify property */
static void BLE_ClientMgr_testNotifyCallback( BLERemoteCharacteristic* pBLERemoteCharacteristic,
                                uint8_t* pData,
                                size_t length,
                                bool isNotify);
/* Init BLE */								
static void BLE_ClientMgr_initBLEService();

/* BLE communication manager main function */
static void BLE_ClientMgr_communicationManagerMainFunction();

/* Check that the client is connected and charactreisitc is valid */
static void BLE_ClientMgr_checkCharacterisitc();
/* If connection is lost, restart scan */
static void BLE_ClientMgr_restartScan();
/* If the server is found try to connect */
static void BLE_ClientMgr_establishCommunication();




/* Callback function that gets called, when another device's advertisement has been received */
class BLE_ClientMgr_AdvertisedDeviceCallbacks : public BLEAdvertisedDeviceCallbacks {
...
void setup() 
{
    ...
    /* Init BLE device */
	BLE_ClientMgr_initBLEService();
	
}

void loop() 
{
    BLE_ClientMgr_communicationManagerMainFunction();
    delay(1000); 
}
...
static bool BLE_ClientMgr_connectToServer(BLEAddress pAddress) 
{
    ...
    testCharacteristic->registerForNotify(BLE_ClientMgr_testNotifyCallback);
    return true;
}
...
static void BLE_ClientMgr_initBLEService()
{
    ...
    pBLEScan->setAdvertisedDeviceCallbacks(new BLE_ClientMgr_AdvertisedDeviceCallbacks());
...
static void BLE_ClientMgr_establishCommunication()
{
    ...
    {
        if (BLE_ClientMgr_connectToServer(*pServerAddress)) 
        {
...
static void BLE_ClientMgr_communicationManagerMainFunction()
{
    ...
    BLE_ClientMgr_checkCharacterisitc();
    /* 2. If connection is lost, restart scan */ 
    BLE_ClientMgr_restartScan();
    /* 3. If the server is found try to connect */ 
    BLE_ClientMgr_establishCommunication();
}
